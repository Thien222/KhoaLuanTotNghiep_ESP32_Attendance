...
exports.getDashboardStats = async (req, res) => {
  try {
    ...
    // NEW: Get detailed today's attendance list for popup
    ...
    const formattedAttendanceList = todayAttendanceList.map(att => ({
      employeeName: att.employee?.name || 'Unknown',
      employeeId: att.employee?.employeeId || 'N/A',
      department: att.employee?.department || 'N/A',
      checkInTime: att.checkIn?.time ? moment(att.checkIn.time).format('HH:mm') : null,
      checkOutTime: att.checkOut?.time ? moment(att.checkOut.time).format('HH:mm') : null,
      status: att.checkIn?.status === 'late' ? 'late' : (att.checkIn?.time ? 'ontime' : 'absent'),
      lateMinutes: att.lateMinutes || 0,
      actualPenalty: att.actualPenalty || 0
    }));

    // NEW: Get late arrivals details
    const lateArrivals = formattedAttendanceList.filter(att => att.status === 'late');

    // NEW: Calculate total monthly salary
    const [payrollYear, payrollMonth] = currentMonth.split('-').map(Number);
    const monthlyPayrolls = await Payroll.find({
      $or: [
        { year: payrollYear, monthNum: payrollMonth },
        { month: currentMonth }
      ]
    });
    const totalMonthlySalary = monthlyPayrolls.reduce((sum, p) => sum + (p.netSalary || p.totalSalary || 0), 0);
    ...
    res.status(200).json({
      ...
      data: {
        ...
        todayAttendance: {
          ...
          list: formattedAttendanceList  // NEW: Full list for popup
        },
        lateArrivals: {
          count: lateArrivals.length,
          list: lateArrivals  // NEW: Late arrivals details
        },
        totalMonthlySalary: Math.round(totalMonthlySalary),  // NEW: Total salary for current month
