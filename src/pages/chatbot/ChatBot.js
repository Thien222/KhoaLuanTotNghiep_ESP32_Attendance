...
const ChatBot = () => {
  ...
  // (Tùy chọn) nếu bạn lưu role ở localStorage sau đăng nhập, có thể đọc để hiển thị gợi ý phù hợp:
  const role = localStorage.getItem('role') || ''; // 'employee' | 'accountant' | 'manager' | 'admin'
  const suggestions = (role === 'accountant' || role === 'manager' || role === 'admin')
  ...
  const handleSendMessage = async () => {
    ...
    try {
      // Gọi API thật tới backend /api/chat
      const resp = await chatApi.send(text);

      const botMessage = {
        id: Date.now() + 1,
        text: resp?.text || 'Không có phản hồi.',
        sender: 'bot',
        timestamp: new Date()
      };
